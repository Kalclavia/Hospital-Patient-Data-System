...
int main (int argc, char* argv[]) {
	...
	control_channel = new TCPRequestChannel("",port);
	int server_sockfd = 0; 
	
	// Fixed issue from previous commit: server_sockfd should be inside the infinite loop
	// This is because the server must continuously accept connections
	// Fixing this issue caused all tests to pass! W :D

	while (true) {
		server_sockfd = control_channel->accept_conn();
		TCPRequestChannel* this_channel = new TCPRequestChannel(server_sockfd);
		thread(handle_process_loop, this_channel).detach();
	}
